...
var API_ACCESS_KEY = 'AIzaSyBrqn9Rl-MX3yJPDKGXL_Ly7ej6DSC0sRw';
...
function handleClientLoad() {
	  gapi.client.load('youtube', 'v3', function() {
	    $.getScript('https://maps.googleapis.com/maps/api/js?v=3.exp&sensor=false&libraries=places&callback=handleMapsLoad&key=' + API_ACCESS_KEY);
...
function clickedSearchButton() {
  ...
  inputObject.inputLat = $('#latitude').val();
  ...
  request.execute(function(response) {
    ...
    if ('error' in response || !response) {
      ...
      for (var i = 0; i < entryArr.length; i++) {
        ...
        videoResult.videourl = "https://www.youtube.com/embed/" + videoResult.videoId;
...
function generateResultList() {
  ...
  for (var i = 0; i < finalResults2.length; i++) {
    ...
    var row_html= [ '<tr class="videopanel" data-youtubeid="'+finalResults2[i].videoId+'" data-description="'+finalResults2[i].description+'" data-title="'+finalResults2[i].title+'" data-lat="'+finalResults2[i].lat+'" data-lng="'+finalResults2[i].long+'" data-id="'+(i+1)+'" data-videourl="'+finalResults2[i].videourl+'">',          
             '<td id="videotd_'+finalResults2[i].videoId+'" class="h2 text-bold text-gray-more">'+(i+1)+'</td>',
             '<td class="vidoverlay_playvidbtn cursor-pointer"><img src="'+ finalResults2[i].thumbNailURL + '" height="100" width="120"/></td>',
             '<td class="text-sm text-bold"><span class="text-sm text-gray-more">'+finalResults2[i].videoId+'</span><br/>'+finalResults2[i].title+'</td>',
             '<td class="text-sm">'+finalResults2[i].description+'</td>',
...
function initializeMap(inputLat, inputLong) {
  ...
  for (var i = 0; i < finalResults2.length; i++) {
    ...
    var image = { url: '/images/markers-numbered/number_' + imageNumber + '.png'};
    ...
    searchResultMarker.id=i+1;
    searchResultMarker.videourl = finalResults2[i].videourl;
    
    
    markersArray.push(searchResultMarker);
    ...
    google.maps.event.addListener(searchResultMarker, 'click', function() {
		$("#videomodal").modal();
		$("#videomodal_iframe").attr( "src", this.videourl+"?autoplay=1&rel=0&modestbranding=1");
		$("#videomodal_title").html(this.id);
